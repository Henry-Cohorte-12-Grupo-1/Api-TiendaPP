...
const updateCategories = async (req: express.Request, res: express.Response) => {
    ...
    const newCategoriesArr: CategoryAttributes[] = newCategories && newCategories.split(" - ").map((c: CategoryAttributes[]) => { return { name: c } }) 
    const oldCategoriesArr: CategoryAttributes[] = oldCategories && oldCategories.split(" - ").map((c: CategoryAttributes[]) => { return { name: c } })
 
    //recibe un array de strings con los nombres de las categorias
    //en ese array no deberian haber categorias repetidas
    newCategoriesArr && newCategoriesArr.forEach(async (category: CategoryAttributes) => {
    ...
    oldCategoriesArr && oldCategoriesArr.forEach(async (category: CategoryAttributes) => {
